...
### Included by Emerson Barea (emerson.barea@gmail.com)
# Setting installation path
INSTALLPATH="$(dirname -- "$(readlink -f -- "$0")" | sed 's/mn-ccnx.*//')"
...
OVS_SRC=$INSTALLPATH/openvswitch
...
function mn_deps {
    ...
    sudo su -c "cat $INSTALLPATH/mn-ccnx/util/sysctl_addon >> /etc/sysctl.conf"

    # Load new sysctl settings:
    sudo sysctl -p

    echo "Installing Mininet/Mini-CCNx core"
    pushd $INSTALLPATH/mn-ccnx
    sudo make install
    popd
}
...
function of {
    ...
    cd $INSTALLPATH/openflow

    # Patch controller to handle more than 16 switches
    patch -p1 < $INSTALLPATH/mn-ccnx/util/openflow-patches/controller.patch
...
function wireshark {
    ...
    if [ "$DIST" = "Ubuntu" ] && [ "$RELEASE" != "10.04" ]; then
        ...
        cd $INSTALLPATH/openflow/utilities/wireshark_dissectors/openflow
        make
        sudo make install
    fi

    # Copy coloring rules: OF is white-on-blue:
    mkdir -p $INSTALLPATH/.wireshark
    cp $INSTALLPATH/mn-ccnx/util/colorfilters ~/.wireshark
}
...
function nox {
    ...
    git am $INSTALLPATH/mn-ccnx/util/nox-patches/*tutorial-port-nox-destiny*.patch
    if [ "$DIST" = "Ubuntu" ] && [ `expr $RELEASE '>=' 12.04` = 1 ]; then
        git am $INSTALLPATH/mn-ccnx/util/nox-patches/*nox-ubuntu12-hacks.patch
    fi
...
function cbench {
    ...
    sh boot.sh
    ./configure --with-openflow-src-dir=$INSTALLPATH/openflow
    make
